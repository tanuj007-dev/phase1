...
function Counter({ end, duration = 2000, suffix = '', label, delay = 0, Icon }) {
  ...
  return (
    <div
      ref={ref}
      className={`flex flex-row items-center gap-3 text-left transition-all duration-1000 sm:gap-5 md:gap-4 ${
        hasTriggered
          ? 'translate-y-0 opacity-100'
          : 'translate-y-8 opacity-0 sm:translate-y-12'
      }`}
    >
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        ...
        <span
          className="mt-0.5 text-xs font-normal sm:text-sm md:text-base"
          style={{ color: '#4595EE' }}
        >
          {label}
        </span>
...
export default function StatsSection() {
  return (
    <section className="relative z-10 w-full border-t-2 border-transparent bg-white px-4 py-12 sm:px-6 sm:py-16 md:py-20 lg:py-10">
